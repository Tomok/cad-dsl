...
/// Helper function to parse with timeout and source text access (for rune blocks)
/// This prevents tests from hanging indefinitely if there's infinite recursion
///
/// Note: input must be 'static for thread safety
pub fn parse_with_timeout_and_source<T: Send + 'static>(
    input: &'static str,
    ...
    std::thread::spawn(move || {
        let result = parse_fn(tokens_static, input);
        let _ = tx.send(result);
    });
